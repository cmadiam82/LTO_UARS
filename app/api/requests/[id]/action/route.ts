...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await transaction(async (client) => {
      ...
      await client.query(`UPDATE uars.access_requests SET status=$1,assigned_role=$2,implementation_id=COALESCE($3,implementation_id),closed_at=CASE WHEN $1='CLOSED' THEN now() ELSE closed_at END,updated_at=now() WHERE id=$4`, [transition.next,transition.nextRole,implementationId,id]);
