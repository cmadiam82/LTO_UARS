...
export async function POST(request: Request) {
  ...
  await query(`INSERT INTO uars.policy_acceptances (user_id,policy_version,policy_hash,ip_address,user_agent) VALUES ($1,$2,$3,$4,$5) ON CONFLICT (user_id,policy_version) DO NOTHING`, [user.id,POLICY_VERSION,POLICY_HASH,forwarded||null,request.headers.get("user-agent")?.slice(0,500)||null]);
