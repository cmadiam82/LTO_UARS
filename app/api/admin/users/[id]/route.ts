...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return transaction(async (client) => {
    ...
    const updated = await client.query(`UPDATE uars.users SET role=$1,is_active=$2,office=COALESCE($3,office),office_id=COALESCE($4,office_id),region_code=COALESCE($5,region_code),agency_code=COALESCE($6,agency_code),full_name=COALESCE(NULLIF($7,''),full_name),employee_id=COALESCE(NULLIF($8,''),employee_id),email=COALESCE(NULLIF($9,''),email),position=COALESCE($10,position),contact_info=COALESCE($11,contact_info),updated_at=now() WHERE id=$12 RETURNING id,username,full_name,employee_id,email,office,office_id,region_code,agency_code,position,contact_info,role,identity_provider,is_active,must_change_password,created_at,updated_at`, [role,isActive,location?.office||null,location?.id||null,location?.region_code||null,location?.agency_code||null,body.fullName||null,body.employeeId||null,body.email||null,body.position??null,body.contactInfo??null,id]);
